...
export default function Index() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f8fafb] text-[#0a2543]">
      ...
      <section id="difference" className="bg-white px-5 py-20 sm:px-8 lg:px-10 lg:py-28"><div className="mx-auto grid max-w-[1380px] gap-14 lg:grid-cols-[1fr_1.15fr] lg:gap-24"><div><SectionLabel>The GTK Projects difference</SectionLabel><h2 className="max-w-[500px] text-5xl font-semibold leading-[0.95] tracking-[-0.065em] text-[#0a2543] sm:text-7xl">Who<br /><span className="text-[#0ba69d]">we are.</span></h2><div className="mt-10 max-w-[475px] border-l-4 border-[#f2d369] bg-[#f5f7f8] p-6 text-sm italic leading-6 text-[#40515b]">“We started GTK Projects because homeowners keep telling us the same thing: big renovation companies quote like they’re building a shopping mall for a leaking gutter. We fix that.”<span className="mt-3 block not-italic text-[10px] font-bold uppercase tracking-[0.13em] text-[#0ba69d]">— GTK Projects Team</span></div><div className="mt-8 space-y-5">{[["Honest & Affordable", "Fair, transparent pricing built for real household and small-business budgets, not corporate margins."], ["No Job Too Small", "From hanging a curtain rail to a full house extension, every job gets the same care and attention."], ["Part of GTK Services Group", "Backed by the same professionalism and reliability as GTK People Partners, our sister company."]].map(([title, text], index) => <div key={title} className="flex gap-4"><span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#d8f4ed] text-xs font-bold text-[#0ba69d]">{index === 0 ? "✓" : index === 1 ? "▣" : "★"}</span><div><h3 className="text-sm font-semibold text-[#0a2543]">{title}</h3><p className="mt-1 max-w-[390px] text-xs leading-5 text-[#68767d]">{text}</p></div></div>)}</div></div><div className="relative min-h-[470px] lg:mt-12"><div className="absolute -right-3 -top-3 h-32 w-32 border-r-2 border-t-2 border-[#f2d369]" /><img src={imageUrls.extension} alt="Modern residential extension and outdoor area" className="h-[430px] w-[82%] object-cover object-center sm:h-[520px]" /><div className="absolute bottom-0 right-0 w-[46%] bg-[#0ba69d] p-5 text-white sm:p-6"><div className="grid grid-cols-2 gap-2 text-center text-[9px] font-bold uppercase leading-3 tracking-[0.09em]"><span><strong className="mb-1 block text-2xl text-[#f2d369]">✓</strong>Honest<br />quotes</span><span><strong className="mb-1 block text-2xl text-[#f2d369]">⌂</strong>Small<br />jobs</span></div></div></div></div></section>
      ...
      <section className="bg-white px-5 py-20 sm:px-8 lg:px-10 lg:py-28"><div className="mx-auto max-w-[1380px]"><SectionLabel>Why choose GTK Projects</SectionLabel><h2 className="max-w-[760px] text-5xl font-semibold leading-[0.92] tracking-[-0.065em] sm:text-7xl">Built to fit your budget,<br /><span className="text-[#0ba69d]">not break it.</span></h2><div className="mt-14 grid gap-x-14 gap-y-10 md:grid-cols-2">{reasons.map(([number, title, text]) => <article key={number} className="grid grid-cols-[48px_1fr] gap-3"><span className="text-3xl font-semibold tracking-[-0.07em] text-[#bce5dd]">{number}</span><div><h3 className="text-sm font-semibold text-[#0a2543]">{title}</h3><p className="mt-3 max-w-[380px] text-xs leading-5 text-[#68767d]">{text}</p></div></article>)}</div><div className="mt-14 flex items-center gap-4 bg-[#0e315a] px-5 py-4 text-xs text-white sm:px-7"><MapPin size={24} className="shrink-0 text-[#0ba69d]" /><span>We operate across the <strong>Greater Johannesburg area</strong>, with projects taken on beyond borders on request.</span></div></div></section>

      <section id="process" className="bg-[#f1f4f6] px-5 py-20 sm:px-8 lg:px-10 lg:py-28"><div className="mx-auto max-w-[1380px]"><SectionLabel>How we work</SectionLabel><h2 className="max-w-[650px] text-5xl font-semibold leading-[0.92] tracking-[-0.065em] sm:text-7xl">From quote<br /><span className="text-[#0ba69d]">to handover.</span></h2><div className="relative mt-14 grid gap-3 md:grid-cols-5">{processSteps.map(([number, title, text], index) => <article key={number} className="relative bg-white p-5 sm:min-h-[250px] sm:p-6"><div className="flex items-start justify-between"><span className="flex h-11 w-11 items-center justify-center rounded-full bg-[#0ba69d] text-white">{index === 4 ? <Check size={22} /> : index === 1 ? <House size={19} /> : index === 2 ? <span className="text-lg">≋</span> : index === 3 ? <Hammer size={19} /> : <Phone size={18} />}</span><span className="text-[11px] font-bold text-[#f2d369]">{number}</span></div><h3 className="mt-8 text-[13px] font-semibold leading-4 text-[#0a2543]">{title}</h3><p className="mt-3 text-[11px] leading-5 text-[#68767d]">{text}</p>{index < processSteps.length - 1 && <span className="absolute -right-3 top-[41px] z-10 hidden h-px w-6 bg-[#91a5ac] md:block" />}</article>)}</div></div></section>
